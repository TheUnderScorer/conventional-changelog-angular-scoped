...
**This package is an unnoficial fork that filters commits by scope.**

## Filtering

Commits are filtered by scope and package name.

Assuming we have following package.json:
```json
{
  "name": "my-package",
  "main": "index.js",
  "release": {
    "tagFormat": "my-package-${version}",
    "plugins": [
      [
...
and we have following commits:

```
feat(my-package): Some new feature
feat(another-package): Another feature
feat: New feature for all packages
```

When we run `npm run release` in `my-package` directory only following commits will be taken into account:
...
thus allowing us to release all packages separately if we are working in monorepo.
For now it only supports scope that matches package name (or empty scope). Perhaps in the future I will add a feature to provide custom matchers for scope.
